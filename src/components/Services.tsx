...
import ServiceVectorHeader from './ServiceVectorHeader';
...
export default function Services() {
  ...
  useEffect(() => {
    ...
    const loadServices = async () => {
      try {
        // Cargar desde Supabase para usar los IDs UUID reales,
        // consistentes con los que espera ReservationModal.
  ...
  const handleCategoryChange = (category: string) => {
    ...
    navigate(`/services?category=${category}`, { replace: true });
  };
...
function ServiceCard({ service, onReserve }: { service: Service; onReserve: (serviceId: string, optionId: string) => void }) {
  ...
  // Keep selectedOption in sync if service changes
  useEffect(() => {
    ...
    <div className="bg-white rounded-3xl overflow-hidden border border-brand-outline/20 hover:border-brand-primary/30 transition-all duration-300">
      ...
      <div className="p-6 space-y-4">
        {(() => {
          ...
          const titleFontSize = displayName.length > 22 
          ...
          return (
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="text-right shrink-0">
                <span className="text-base font-semibold text-brand-primary block tracking-tight">${selectedOption.price.toFixed(2)}</span>
              </div>
            </div>
          );
        })()}

        {/* Modal/Option Selector Tabs (Clásico vs Premium) */}
                  ...
                  >
                    <span 
                      className={`w-2 h-2 rounded-full transition-all duration-200 shrink-0 ${
                        isActive 
                          ? 'bg-brand-primary scale-110 ring-2 ring-brand-primary/30' 
